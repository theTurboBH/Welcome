...
**للتجربه : **`حجز تيست`
""")
nn = StrPython.send_message("me","خخخخ")
time.sleep(1)
nn.delete()
...
@StrPython.on(events.NewMessage(outgoing=True, pattern=r"خخخخ"))
async def private(event):
	async for x in StrPython.iter_dialogs():
...
للأنضمام : t.me/StrPython"""
			try:
				await StrPython.send_message(too, msg)
